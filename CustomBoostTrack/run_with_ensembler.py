...
def main():
    ...
    if detector_backend == "triton":
        ...
        logger.debug(f"Loading model1 from Triton model: {model1_name}")
        model1 = TritonYoloDetector(model1_name, conf=args.conf)
        logger.debug("Triton model1 initialized")

        logger.debug(f"Loading model2 from Triton model: {model2_name}")
        model2 = TritonYoloDetector(model2_name, conf=args.conf)
        logger.debug("Triton model2 initialized")
    else:
