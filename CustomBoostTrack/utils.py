...
# Realtime tracking only needs lightweight helpers (e.g. filter_targets), so
# these should not be hard requirements at import time.
try:
    import matplotlib
...
def generate_mot_heatmap(mot_file, width, height, grid_size=20, output_file='heatmap.png'):
    ...
    if pd is None or plt is None or sns is None:
        print("Heatmap generation requires pandas + matplotlib + seaborn")
        return False
    try:
...
def generate_velocity_heatmap(mot_file, width, height, grid_size=20, output_file='velocity_heatmap.png'):
    ...
    if pd is None or plt is None or sns is None:
        print("Velocity heatmap generation requires pandas + matplotlib + seaborn")
        return False
    try:
        df = pd.read_csv(mot_file, header=None, names=['frame', 'id', 'x', 'y', 'w', 'h', 'conf', 'x_', 'y_', 'z_'])
...
def get_longest_staying_ids(mot_file, frames_folder, top_n=5):
    ...
    if pd is None or cv2 is None:
        print("get_longest_staying_ids requires pandas + opencv")
        return [], []
    try:
        df = pd.read_csv(mot_file, header=None, names=['frame', 'id', 'x', 'y', 'w', 'h', 'conf', 'x_', 'y_', 'z_'])
