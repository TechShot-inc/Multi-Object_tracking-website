document.addEventListener('DOMContentLoaded', () => {
    ...
    if (roiCanvasUpload && uploadPreview) {
        ...
        // Update ROI canvas dimensions when video is loaded
        function resizeRoiCanvas() {
            if (uploadPreview.videoWidth && uploadPreview.videoHeight) {
                ...
                // Draw first frame on canvas for ROI selection
                setTimeout(() => {
                    roiContextUpload.drawImage(uploadPreview, 0, 0, roiCanvasUpload.width, roiCanvasUpload.height);
                }, 100);
            }
    ...
    function startDrawingROI(e) {
        // Only allow drawing if video is loaded
        if (!uploadPreview.videoWidth) {
            showModal('Please wait for the video to load before selecting ROI.');
            return;
        }
        // Get scaling factor based on canvas size vs. displayed size
        ...
        // Calculate scaled coordinates and ensure they are integers
        ...
        console.log('Canvas dimensions:', roiCanvasUpload.width, 'x', roiCanvasUpload.height);
        console.log('Video dimensions:', videoWidth, 'x', videoHeight);
    ...
    function finishDrawingROI(e) {
        ...
        // Set ROI with correct coordinates - ensure all values are integers
        window.roiUpload = {
        ...
        // Calculate minimum size as 5% of video dimensions
        const minWidth = Math.floor(videoWidth * 0.05);
        const minHeight = Math.floor(videoHeight * 0.05);
        
        console.log('ROI dimensions:', window.roiUpload);
        console.log('Minimum required dimensions:', minWidth, 'x', minHeight);
        console.log('Video dimensions:', videoWidth, 'x', videoHeight);
        console.log('Raw coordinates:', rawX, rawY);
    ...
    uploadForm.addEventListener('submit', async (e) => {
        ...
        if (window.roiUpload && window.roiUpload.width > 0 && window.roiUpload.height > 0) {
            // Send absolute pixel values instead of relative coordinates
            const roiData = {
        ...
        try {
            ...
            if (data.status === 'success') {
                ...
                // Start polling with filename instead of resultId
    ...
    async function pollStatus(filename) {
        try {
            ...
            if (data.status === 'completed') {
                // Verify that the video file exists and is ready
            ...
            if (error.message.includes('Video not ready') || error.message.includes('Video processing not complete')) {
                // If video is not ready, continue polling
                await new Promise(resolve => setTimeout(resolve, 1000));
                await pollStatus(filename);
            } else {
                statusMessage.textContent = `Error: ${error.message}`;
    ...
    async function displayResults(filename) {
        try {
            ...
            // Setup download buttons with immediate download
            ...
            downloadAnnotationsButton.onclick = (e) => {
                ...
                link.download = `${filename.replace('.mp4', '')}_annotations.json`;
                document.body.appendChild(link);
                link.click();
                document.body.removeChild(link);
            };
            ...
            if (error.message.includes('Video loading timed out') || error.message.includes('Failed to load video')) {
                // If video loading failed, continue polling
                await new Promise(resolve => setTimeout(resolve, 1000));
                await pollStatus(filename);
            } else {
                statusMessage.textContent = `Error displaying results: ${error.message}`;
            }
        }
    }
