...
class RealTimeTrackingService:
    ...
    def process_frame(self, frame, roi=None, line=None):
        ...
        try:
            ...
            for det in detections:
                print(
                    f"Detection: id={det['id']}, class={det.get('class')}, x={det['x']}, y={det['y']}, w={det['width']}, h={det['height']}, conf={det['confidence']}"
                )
            ...
            if line and isinstance(line, dict) and all(k in line for k in ["position", "x"]):
                try:
                    ...
                    for det in detections:
                        ...
                        area_on_inside = self.calculate_area_on_side(
                            det, line_x, inside_side, frame_width, frame_height
                        )
                        ...
                        print(
                            f"Detection id={det['id']}: area_on_inside={area_on_inside}, total_area={total_area}, ratio={area_ratio:.2f}"
                        )
                        ...
                        if area_ratio >= 0.75:
                            inside_count += 1
                            print(
                                f"Person ≥75% on inside (side={inside_side}, ratio={area_ratio:.2f}), inside_count={inside_count}"
                            )
                        else:
                            outside_count += 1
                            print(
                                f"Person <75% on inside (side={inside_side}, ratio={area_ratio:.2f}), outside_count={outside_count}"
                            )
            ...
            if roi and isinstance(roi, dict) and all(k in roi for k in ["x", "y", "width", "height"]):
                ...
                cv2.rectangle(
                    frame, (x_roi, y_roi - label_size[1] - 5), (x_roi + label_size[0] + 10, y_roi), border_color, -1
                )
                cv2.putText(frame, label, (x_roi + 5, y_roi - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
            ...
            for det in detections:
                ...
                label_size, baseline = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 0.5, 2)
                cv2.rectangle(frame, (x1, y1 - label_size[1] - baseline - 2), (x1 + label_size[0] + 2, y1), color, -1)
                cv2.putText(
                    frame, label, (x1 + 1, y1 - baseline - 1), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1
                )
