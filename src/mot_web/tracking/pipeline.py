...
def run_video(input_path: Path, output_dir: Path, params: dict[str, Any]) -> None:
    ...
    if not out.isOpened():
        logger.error(f"Failed to create output video writer")
        cap.release()
...
def _convert_video_for_browser(video_path: Path) -> None:
    ...
    if not ffmpeg_path:
        logger.warning("ffmpeg not found, video may not play in browser. Install ffmpeg or imageio-ffmpeg for better compatibility.")
        return
...
def _create_stub_output(input_path: Path, output_dir: Path, params: dict[str, Any], error: str = "") -> None:
